...
def create_base_dict(letters, combination_length):
    ...
    my_list = list(product(letters,
                           repeat=combination_length))  # create all 4 (or 2 or 6) letter combination list. ex: ('A', 'C', 'G', 'U'), ..
    final_list = [None] * len(my_list)

    for i in range(len(my_list)):  # get rid of the inner lists and bring the strings together
        my_str = ""
        temp_list = my_list[i]
...
def dense_encoder(target_RNA, reverse_DNA, pairs, encoding='doublet'):
    ...
    for j in range(len(target_RNA)):  # go through all sequences
        ...
        for i in range(len(RNA) - end):  # go through stacks
            ...
            my_index = 0
            for pair, index in pairs.items():  # get the index of the base pair (1 to 256)
                if pair == base_pair:
                    my_index = index
                    break
...
class off_tar_read(object):
    ...
    def __init__(self, file_path, pairs):  # passing arguments to class
        ...
        self.target = self.read_file.loc[:, 'sequence']  # list of the target RNA seq/on-target sites
        self.off_tar = self.read_file.loc[:, 'Target sequence']  # list of the off_target DNA seq
        # target -> the gRNA sequence
        # off-target -> the DNA sequence gRNA binds to

        self.labels = np.asarray(self.read_file.loc[:, 'class'])  # get the labels and convert to array
        self.pairs = pairs
...
class base_pair(object):
    ...
    def create_dict(self, encoding='doublet'):
        ...
        list_of_letters = create_base_dict(self.letters, combination)  # get all possible length X combinations
        self.pairs = dict(zip(list_of_letters, temp))  # create the code numbers for encoding
        return self.pairs
...
class GetSeq(object):
    ...
    def get_reverse(self, encoding="doublet"):
        ...
        on_target = []  # Get an empty 2D array, [] for _ in range(self.input_seq.shape[0])
        off_target = []
